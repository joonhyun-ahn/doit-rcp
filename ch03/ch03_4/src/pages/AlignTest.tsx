...
export default function AlignTest() {
  const boxes = D.range(0, 5).map((index) => {
    return (
      <div key={index} className={`text-white ${index % 2 === 0 ? 'text-xs' : 'text-lg'} bg-black w-8 h-8 m-1`}>
        {index}
      </div>
    );
  });
  const boxesForStretch = D.range(0, 5).map((index) => {
  ...
  const items = ['items-start', 'items-center', 'items-end', 'items-baseline'].map((item) => (
