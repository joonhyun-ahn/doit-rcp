...
import { Div, Icon, type DivProps } from '../components';
...
const Card: FC<CardProps> = ({ card, ...props }) => {
  ...
  const icons = ['home', 'search', 'settings', 'favorite'].map((name) => <Icon key={name} name={name} className="mr-2 text-3xl" />);
    ...
    <Div {...props}>
      <div className="flex flex-col">
        ...
        <Div className="p-4" minHeight="16rem" height="16rem" maxHeight="16rem">
          ...
          <Div className="flex flex-row items-center justify-around p-2 mt-2 text-red-500">{icons}</Div>
        </Div>
      </div>
    </Div>
  );
